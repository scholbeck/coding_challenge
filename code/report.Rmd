...
- Behalte jeweils einen Feature-Vektor je Kunden (alle identisch) und join mit gesamten Versicherungsanspruch.
...
- **Fazit:** Zwar lassen lineare Korrelationen nicht ausschließen, dass es nichtlineare Zusammenhänge gibt, jedoch sind die vorhandenen linearen Assoziationen nicht sehr vielversprechend für ein zu trainierendes Modell, welches einen multivariaten Zusammenhang beschreibt.


## Modellierung

### Extremwerte der Versicherungsansprüche

- Aufgrund der extrem rechtsschiefen Verteilung der Zielvariablen bietet sich eine Modellierung des Verteilungsrandes (mit sehr wenigen Werten) per Extremwertverteilung an (heavy Tail).
...
### "Reguläre" Werte der Versicherungsansprüche

- Für Ansprüche kleiner als 250.000 EUR werden nun mehrere Regressionsmodelle via mlr3 trainiert.

- Ein Modell ohne Features (Durchschnitt) und ein lineares Modell mit allen Features wird als Baseline zur Referenz trainiert. Darüber hinaus trainieren wir einen Regressionsbaum (CART), ein k-nearest-Neighbor-Modell mit 7 Nachbarpunkten, eine Support Vector Machine, sowie einen Random Forest.

- Hierzu wird ein Benchmark von 6 Learnern erstellt (5-fach Kreuzvalidierung sowie One-Hot-Encoding kategorieller Features).

- Zur Illustration wird der Komplexitätsparameter bei rpart innerhalb des Benchmarks getuned.

- Auf eine aufwendige Hyperparameteroptimierung wird verzichtet, um den Rechenaufwand zu begrenzen. Darüber hinaus sollte ein Random Forest auch ohne Tuning passable Performance zeigen.
...
- Nicht überraschend finden wir kein einziges passendes Modell; der reine Durchschnitt der Zielvariable als Baseline zeigt eine ähnliche Performance wie CART oder ein Random Forest!

- Als Illustration dient die Vorhersage des linearen Modells mit allen Features auf ungesehenen Testdaten, welches ähnlich wie alle anderen Modelle abschneidet: Wir verzeichnen extrem hohe Residuen.

- Dementsprechend erübrigt sich eine Analyse von Feature-Importance und Feature-Effects innerhalb eines "besten" Modells.
...
ggplot(predictions, aes(x = truth, y = response)) +
  geom_point() +
  theme_bw() +
  xlab("Beobachteter Versicherungsanspruch je Kunde je Jahr") +
  ylab("Vorhersage")
```

- Ich schlussfolgere, dass die Schätzung einer bedingten Dichte aufgrund fehlender Zusammenhänge hier nicht zum Ziel führt.

- Dass weitere Ausreißer (beispielsweise im Bereich von 9000 EUR und 250000) das Modell verzerren, erscheint auch unplausibel, da robuste Techniken wie CART oder Random Forests ebenso sehr schlecht vorhersagen.
...
- Für die Wahrscheinlichkeiten für einen Versicherungsanspruch je Kunde je Jahr haben wir nun zwei unterschiedliche Verteilungen mit geschätzten a-priori-Wahrscheinlichkeiten von 99.5%, dass ein Versicherungsanspruch < 250.000 EUR ist, sowie von 0.5%, dass ein Versicherungsanspruch > 250.000 EUR ist.

```{r}
# unter 250.000
p = nrow(model_data) / nrow(insurance_data)
p
# über 250.000
1-p
```

- Die zusammgensetzte Mischverteilung ist ein durch die a-priori-Wahrscheinlichkeiten gewichteter Durchschnitt der Verteilungen.

$$
\begin{equation}
  \text{CDF}(\text{ClaimPerYear} = x) = 0.995 \cdot \text{CDF}_{\text{empirisch}}(x) + 0.005 \cdot          
  \text{CDF}_{\text{Pareto}}(x)
\end{equation}
$$

## Wirtschaftliche Umsetzung und Handlungsempfehlungen

- Die Versicherungsgesellschaft kann nun erwartete Verluste auf die Kunden umlegen.
...
- Die zusammgesetzte Verteilung kann unter Umständen noch weiter optimiert werden: 
  - Schätzung theoretischer Verteilungen für "reguläre" Versicherungsansprüche.
  - Weitere Optimierung der Threshold.
  - Einteilung der Versicherungsansprüche je Kunde je Jahr in mehr Segmente.
  
- Darüber hinaus können wir andere Modellierungsansätze prüfen, die für die Versicherungsgesellschaft relevant sein können:
  - Die Wahrscheinlichkeit, dass ein Kunde überhaupt einen Schaden geltend macht.
  - Die Häufigkeit eines Schadens je Kunde je Jahr.